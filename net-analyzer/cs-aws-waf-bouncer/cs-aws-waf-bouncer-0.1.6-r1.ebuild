...
src_unpack() {
	default
	mv "${WORKDIR}/vendor" "${WORKDIR}/cs-aws-waf-bouncer-${PV}/"
}
...
src_install() {
	...
	newconfd "${FILESDIR}/${PN}.conf" crowdsec-aws-waf-bouncer
}

pkg_postinst() {
	elog "Before running your crowdsec aws waf bouncer, you will need to:"
	elog " - adjust the config in /etc/crowdsec/crowdsec-aws-waf-bouncer.yaml"
